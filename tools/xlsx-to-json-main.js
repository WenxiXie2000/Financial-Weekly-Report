...
import { convertSheets } from "../js/xlsx2json/core.js";
...
function createResultCard({ title, fileName, data, diagnostics }) {
  ...
  if (Array.isArray(diagnostics) && diagnostics.length) {
    const totalIssues = diagnostics.reduce((sum, diag) => {
      ...
      return (
        sum + diag.items.filter((item) => item && item.matched === false).length
      );
    }, 0);
    ...
    diagnostics.forEach((diag) => {
      ...
      diag.items.slice(0, 6).forEach((item) => {
        ...
        status.className = item.matched
          ? "diagnostics-chip"
          : "diagnostics-chip warn";
...
function renderResults(results) {
  ...
  if (!results.length) {
    ...
    empty.textContent =
      "未找到可解析的工作表，请确认 Excel 文件的 Sheet 名称。";
...
async function handleFile(event) {
  ...
  const aggregated = Array.from(outputsByFile.entries()).map(
    ([fileName, data]) => {
      const sources = Array.isArray(data?.meta?.sourceSheets)
        ? data.meta.sourceSheets
        : [];
      const title = sources.length
        ? sources.join(" + ")
        : data?.meta?.sourceSheet || fileName;
