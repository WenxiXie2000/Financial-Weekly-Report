/**
 * 债券利率视图：展示中票曲线与各品类发行 Top5 概览。
 * - 数据来源：bond_yield.json 中的 { series, top5_latest }。
 * - top5_latest 契约：{ [group_key]: { date: string, rows: [{rank, issuer, size_yi, term, coupon_pct}] } }。
 * - 本模块当前聚焦 Top5 表格，若后续扩展折线图，可复用 common-charts.renderLineChart。
 */
...
/**
 * 渲染债券利率 Top5 卡片。
 * @param {HTMLElement} mount - 页面中用于承载卡片的 DOM 容器。
...
export async function renderBondYield(mount) {
  ...
  Object.entries(top).forEach(([g, block]) => {
    ...
    rows.forEach((r) => {
      ...
      tr.innerHTML = `
        <td style="padding:6px;border-bottom:1px solid var(--border)">${r.rank ?? '--'}</td>
        <td style="padding:6px;border-bottom:1px solid var(--border)">${r.issuer ?? '--'}</td>
        <td style="padding:6px;border-bottom:1px solid var(--border);text-align:right">${sizeDisp}</td>
        <td style="padding:6px;border-bottom:1px solid var(--border)">${r.term ?? '--'}</td>
