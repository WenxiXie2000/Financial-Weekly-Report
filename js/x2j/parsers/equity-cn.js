...
export default function parseEquityCn(
  ...
  const indices = Array.isArray(profile?.indices) ? profile.indices : [];
  ...
  indices.forEach((indexDef) => {
    const label =
      (indexDef && (indexDef.label || indexDef.alias || indexDef.key)) ||
      String(indexDef || '').trim();
    const matcherSeed =
      indexDef && (indexDef.alias || indexDef.label || indexDef.key)
        ? indexDef.alias || indexDef.label || indexDef.key
        : label;
    if (!label || !matcherSeed) return;

    Object.entries(metrics).forEach(([metricKey, meta]) => {
      if (!meta) return;
      const matcher = typeof meta.matcher === 'function' ? meta.matcher(matcherSeed) : meta.matcher;
      if (!matcher) return;

      const colIdx = findColIndex(headerIndex, matcher);
      recordHit(`${label}.${metricKey}`, matcher, colIdx);
      if (colIdx < 0) return;

      trackedColumns.set(colIdx, meta);
      const metricLabel = meta.label || metricKey;
      const name = `${label} ${metricLabel}`;
